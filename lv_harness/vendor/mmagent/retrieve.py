...
# retrieve by clip
def retrieve_from_videograph(video_graph, query, topk=5, mode='max', threshold=0, before_clip=None, allowed_clips=None):
    top_clips = []
    ...
    # before_clip and allowed_clips can take effect simultaneously
    top_clips = []
    for clip_id, score in sorted_clips:
        ...
        if before_clip is not None and clip_id > before_clip:
            continue
        if allowed_clips is not None and clip_id not in allowed_clips:
            continue
        top_clips.append(clip_id)
        if len(top_clips) >= topk:
            break
...
def select_queries(action_content, responses):
    ...
    history_embeddings = parallel_get_embedding("text-embedding-v4", history_queries)[0]
    
    queries = action_content
    embeddings = parallel_get_embedding("text-embedding-v4", queries)[0]
...
def search(video_graph, query, current_clips, topk=5, mode='max', threshold=0, mem_wise=False, before_clip=None, episodic_only=False, allowed_clips=None):
    
    allowed_set = set(allowed_clips) if allowed_clips is not None else None
    top_clips, clip_scores, nodes = retrieve_from_videograph(video_graph, query, topk, mode, threshold, before_clip,allowed_clips=allowed_set)
    ...
    if mem_wise:
        ...
        for top_node, _ in nodes:
            ...
            if before_clip is not None and clip_id > before_clip:
                continue
            if allowed_set is not None and clip_id not in allowed_set:
                continue
...
def calculate_similarity(mem, query, related_nodes):
    ...
    query_embedding = np.array(get_embedding_with_retry("text-embedding-v4", query)[0]).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    video_graph_path = "/path/to/mmagent/data/mems/CZ_1/Efk3K4epEzg_30_5_-1_10_20_0.3_0.6.pkl"
    video_graph = load_video_graph(video_graph_path)

    question = "Which collection has the highest starting price?"
    answer = answer_with_retrieval(video_graph, question, video_to_base64("/path/to/mmagent/data/video_clips/CZ_1/Efk3K4epEzg/39.mp4"), topk=processing_config["topk"], multiple_queries=processing_config["multiple_queries"], max_retrieval_steps=processing_config["max_retrieval_steps"])
