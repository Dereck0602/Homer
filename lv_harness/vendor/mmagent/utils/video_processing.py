...
def compress_video_fps(base64_video_str, target_fps=5, max_width=1280, max_height=720):
    """Compress a base64-encoded video to target_fps and cap the maximum resolution.

    For videos whose original resolution is already below max_width x max_height, no resolution scaling is done.

    Args:
        base64_video_str (str): base64-encoded video string
        target_fps (int): target frame rate, default 5fps
        max_width (int): max width, default 1280
        max_height (int): max height, default 720

    Returns:
        str: the compressed base64-encoded video string
    """
    video_bytes = base64.b64decode(base64_video_str)
    ...
    try:
        # use ffprobe to get the original video resolution
        probe_cmd = [
            "ffprobe", "-v", "error",
            "-select_streams", "v:0",
        ...
        if probe_result.returncode == 0:
            ...
            if streams:
                orig_w = streams[0].get("width", 0)
                orig_h = streams[0].get("height", 0)
                if orig_w > max_width or orig_h > max_height:
                    need_scale = True
                    logger.info(f"video resolution {orig_w}x{orig_h} exceeds limit {max_width}x{max_height}, will scale")
                else:
                    logger.info(f"video resolution {orig_w}x{orig_h} within limit, skipping scaling")
...
def compress_video_file(video_path, target_fps=5, max_width=1280, max_height=720):
    """Compress a video file's frame rate and cap the maximum resolution, returning the compressed temp file path.

    For videos whose original resolution is already below max_width x max_height, no resolution scaling is done.
    Note: the caller is responsible for cleaning up the returned temp file after use.
    ...
    Returns:
        str: the compressed temp video file path (returns the original path if no compression is needed)
    """
    # use ffprobe to get the original video resolution and frame rate
    probe_cmd = [
        "ffprobe", "-v", "error",
        "-select_streams", "v:0",
    ...
    if probe_result.returncode == 0:
        ...
        if streams:
            orig_w = streams[0].get("width", 0)
            orig_h = streams[0].get("height", 0)
            # parse the frame rate (format like "25/1")
            fps_str = streams[0].get("r_frame_rate", "0/1")
            num, den = fps_str.split("/")
            orig_fps = float(num) / float(den) if float(den) != 0 else 0
            
            if orig_w > max_width or orig_h > max_height:
                need_scale = True
                logger.info(f"video file {video_path} resolution {orig_w}x{orig_h} exceeds limit {max_width}x{max_height}, will scale")
            
            if orig_fps > target_fps:
                need_fps_compress = True
                logger.info(f"video file {video_path} frame rate {orig_fps} exceeds target {target_fps}fps, will compress")
    
    if not need_scale and not need_fps_compress:
        logger.info(f"video file {video_path} needs no compression, returning the original path")
        return video_path
    ...
    try:
        ...
        logger.info(f"video file compression complete: {video_path} -> {tmp_out_path}")
        return tmp_out_path
    except Exception as e:
        # on compression failure, clean up the temp file and return the original path
        if os.path.exists(tmp_out_path):
            os.remove(tmp_out_path)
        logger.error(f"video file compression failed: {e}, using the original file")
        return video_path
