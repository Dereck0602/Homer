...
# moviepy lazy import: loaded only when VideoFileClip etc. are actually used
# from moviepy import *
import re
import ast
import numpy as np
# matplotlib and sklearn lazy import: used only in plotting functions
...
from mmagent._config_loader import get_processing_config, get_memory_config; processing_config = get_processing_config()
...
def plot_cosine_similarity_distribution(embeddings1, embeddings2, save_path=None, max_num=2000):
    ...
    # Compute all similarity combinations between the two sets of embeddings
...
def _ensure_legacy_videograph_pickle_alias():
    """Register the legacy top-level module name used by older VideoGraph pickles."""
    try:
        import sys
        import mmagent.videograph as videograph_module

        sys.modules.setdefault("videograph", videograph_module)
    except Exception as e:
        logger.debug(f"Unable to register legacy videograph pickle alias: {e}")
...
def load_video_graph(video_graph_path):
    ...
    if not os.path.exists(video_graph_path):
        ...
        return None
    _ensure_legacy_videograph_pickle_alias()
